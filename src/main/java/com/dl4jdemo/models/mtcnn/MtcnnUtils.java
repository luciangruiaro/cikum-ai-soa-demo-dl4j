package com.dl4jdemo.models.mtcnn;

import com.dl4jdemo.utils.ArgSortParam;
import com.dl4jdemo.utils.BoundBox;
...
public class MtcnnUtils {
    ...
    public INDArray nms(INDArray boxes, double threshold, boolean isMethodMin) {
        ...
        while (true) {
            ...
            INDArray xx1 = Nd4jUtils.maximum(x1.getDouble(highestIndex), x1.get(otherIndexes));
            INDArray yy1 = Nd4jUtils.maximum(y1.getDouble(highestIndex), y1.get(otherIndexes));
            INDArray xx2 = Nd4jUtils.minimum(x2.getDouble(highestIndex), x2.get(otherIndexes));
            INDArray yy2 = Nd4jUtils.minimum(y2.getDouble(highestIndex), y2.get(otherIndexes));

            // compute the width and height of the bounding box
            INDArray w = Nd4jUtils.maximum(0.0, xx2.sub(xx1).addi(1));
            INDArray h = Nd4jUtils.maximum(0.0, yy2.sub(yy1).addi(1));
            INDArray interArea = w.mul(h);

            // compute the ratio of overlap
            INDArray overlap;
            if (isMethodMin) {
                overlap = interArea.divi(Nd4jUtils.minimum(area.getDouble(highestIndex), area.get(otherIndexes)));
            } else {
                overlap = interArea.divi(area.get(otherIndexes).addi(area.getDouble(highestIndex)).subi(interArea));
            }
            INDArray tmpFitIdx = Nd4jUtils.findFitIndexes(overlap, Conditions.lessThanOrEqual(threshold));
            if (tmpFitIdx == null) {
                break;
            }
    ...
    public INDArray rerec(INDArray totalBoxes, double imgW, double imgH) {
        ...
        INDArray regl = Nd4jUtils.maximum(w, h.dup());
        INDArray lossW = regl.sub(w).muli(0.5);
        INDArray lossH = regl.sub(h).muli(0.5);
        totalBoxes.put(new INDArrayIndex[] { all(), point(0) }, Transforms.floor((Nd4jUtils.maximum(0.0, x1.subi(lossW)))));
        totalBoxes.put(new INDArrayIndex[] { all(), point(1) }, Transforms.floor((Nd4jUtils.maximum(0.0, y1.subi(lossH)))));
        totalBoxes.put(new INDArrayIndex[] { all(), point(2) }, Transforms.floor((Nd4jUtils.minimum(imgW, x2.addi(lossW)))));
        totalBoxes.put(new INDArrayIndex[] { all(), point(3) }, Transforms.floor((Nd4jUtils.minimum(imgH, y2.addi(lossH)))));
        return totalBoxes;
    }
